        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3 flex-wrap">
              <ComplianceBadge complianceLevel={agent.compliance_level} className="bg-white bg-opacity-20 text-white border-white border-opacity-30" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white shadow-lg rounded-xl p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Platform Compliance</h2>
          </div>
          <div className="space-y-4">
            {/* Compliance Level */}
            <div className="p-3 bg-gray-50 rounded-lg">
              <span className="text-sm text-gray-600 block mb-2">Compliance Level</span>
              <ComplianceBadge complianceLevel={agent.compliance_level} className="text-sm" />
              {!agent.compliance_level && (
                <span className="text-xs text-gray-500 mt-1 block">Not analyzed</span>
              )}
            </div>

            {/* Communication Score */}
            {agent.communication_score !== undefined && (
              <div className="p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-gray-600">Platform Integration Score</span>
                  <span className="text-sm font-medium text-gray-900">{agent.communication_score}/100</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div 
                    className={`h-3 rounded-full transition-all duration-500 ${
                      (agent.communication_score || 0) >= 80 ? 'bg-green-500' :
                      (agent.communication_score || 0) >= 40 ? 'bg-amber-500' : 'bg-red-500'
                    }`}
                    style={{ width: `${Math.max(agent.communication_score || 0, 3)}%` }}
                  ></div>
                </div>
                <p className="text-xs text-gray-500 mt-1">
                  {(agent.communication_score || 0) >= 80 ? 'High platform integration' :
                   (agent.communication_score || 0) >= 40 ? 'Moderate platform integration' :
                   (agent.communication_score || 0) > 0 ? 'Low platform integration' : 'Not analyzed for platform features'}
                </p>
              </div>
            )}
            ...
            {/* Security Analysis */}
            <div className="p-3 bg-gray-50 rounded-lg">
              <span className="text-sm text-gray-600 block mb-2">Security Analysis</span>
              {agent.compliance_level ? (
                <div className="flex items-center space-x-2">
                  {agent.compliance_level.toLowerCase() === 'verified' ? (
                    <svg className="w-4 h-4 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                    </svg>
                  ) : agent.compliance_level.toLowerCase() === 'likely' ? (
                  ...
                  <span className="text-sm text-gray-900">
                    {agent.compliance_level.toLowerCase() === 'verified' ? 'Passed security scan' :
                     agent.compliance_level.toLowerCase() === 'likely' ? 'Moderate risk detected' :
                     'Use with caution'}
                  </span>
                </div>
              ) : (
                <span className="text-xs text-gray-500">Security scan not available</span>
              )}
            </div>
